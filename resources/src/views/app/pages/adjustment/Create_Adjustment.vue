<template>
    <div class="main-content">
        <breadcumb
            :page="$t('CreateAdjustment')"
            :folder="$t('ListAdjustments')"
        />
        <div
            v-if="isLoading"
...
export default {
    ...
    methods: {
        ...
        Search_Product(input) {
            ...
            return (this.adjustment.warehouse_id !== "") ? this.products.filter(product =>
                {
                    return (product.name.toLowerCase().includes(input.toLowerCase()) || product.code.toLowerCase().includes(input.toLowerCase()));
                }
            ) : ((input.length < 1) ? [] : []);
        },
        ...
        Verified_Qty(detail, id) {
            for (var i = 0; i < this.details.length; i++) {
                if (this.details[i].detail_id === id) {
                    ...
                    if (
                        detail.type == "sub" &&
                        detail.quantity > detail.current
                    ) {
                        this.makeToast(
                            "warning",
                            this.$t("LowStock"),
